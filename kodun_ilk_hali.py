...
btcdf = nasdaqdatalink.get('BCHAIN/MKPRU', collapse='daily')
...
stringencydf = stringencydf.T.iloc[2:, :]
...
for j in range(1, 2):
    ...
    for city in ['istanbul', 'izmir', 'ankara']:
        citydf = pd.DataFrame()
        url = f'https://api.worldweatheronline.com/premium/v1/past-weather.ashx?date={startst}&enddate={endst}&key=cbfd28cade5c41a3a48105245220806&q={city},turkey&format=json'
...
usdtrydf = investpy.get_currency_cross_historical_data(currency_cross='USD/TRY', from_date='01/01/2019',
